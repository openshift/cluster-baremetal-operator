...
func checkNodeStatus(oc *exutil.CLI, pollIntervalSec time.Duration, pollDurationMinute time.Duration, nodeName string, nodeStatus string) error {
	...
	errNode := wait.PollUntilContextTimeout(context.Background(), pollIntervalSec, pollDurationMinute, false, func(ctx context.Context) (bool, error) {
		output, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("node", nodeName, "-o=jsonpath={.status.conditions[?(@.type==\"Ready\")].status}").Output()
		if err != nil {
			e2e.Logf("Error getting node status: %v", err)
			return false, nil
		}
		if strings.TrimSpace(output) == nodeStatus {
			e2e.Logf("Node %s Ready status is %s", nodeName, output)
			return true, nil
		}
		e2e.Logf("Node %s Ready status: %s, waiting for %s", nodeName, output, nodeStatus)
		return false, nil
	})
	compat_otp.AssertWaitPollNoErr(errNode, fmt.Sprintf("Node %s did not reach Ready status %s", nodeName, nodeStatus))
	return errNode
}
