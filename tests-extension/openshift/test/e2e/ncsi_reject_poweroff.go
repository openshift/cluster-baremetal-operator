...
var _ = g.Describe("[OTP][sig-baremetal] IPI BareMetal", func() {
	...
	g.It("Author:jhajyahy-Longduration-NonPreRelease-Medium-80760-Support for hosts that can be rebooted but not powered off [Disruptive]", func() {
		...
		invalidBMHConfigPath := CopyToFile(bmhYaml, "bmh.yaml")
		...
		compat_otp.ModifyYamlFileContent(invalidBMHConfigPath, []compat_otp.YamlReplace{
			{
				Path:  "metadata.name",
				Value: bmhName,
			},
		...
		compat_otp.By("Create bmh using saved yaml file")
		out, err := oc.AsAdmin().WithoutNamespace().Run("create").Args("-f", invalidBMHConfigPath).Output()
		...
		validBMHConfigPath := CopyToFile(bmhYaml, "bmh.yaml")
		compat_otp.ModifyYamlFileContent(validBMHConfigPath, []compat_otp.YamlReplace{
			{
				Path:  "metadata.name",
				Value: bmhName,
			},
		...
		compat_otp.By("Create bmh using saved yaml file")
		err = oc.AsAdmin().WithoutNamespace().Run("apply").Args("-f", validBMHConfigPath).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
