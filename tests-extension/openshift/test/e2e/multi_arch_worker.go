...
func verifyMultiArchWorker(oc *exutil.CLI, expectedBootMethod string) {
	...
	e2e.Logf("Control plane architecture is: %s", controlPlaneArch.String())
	...
	for _, arm64Node := range arm64WorkerNodes {
		...
		// Get BMC address for logging
		bmcAddress, _ := oc.AsAdmin().WithoutNamespace().Run("get").Args("baremetalhost", "-n", "openshift-machine-api", actualBMHName, "-o=jsonpath={.spec.bmc.address}").Output()

		if bootMethod == expectedBootMethod {
			matchingWorkers = append(matchingWorkers, arm64Node)
			if bootMethod == "vmedia" {
				e2e.Logf("*** Worker node %s was added with VMEDIA boot (BMC address: %s) ***", arm64Node, bmcAddress)
			} else {
				e2e.Logf("*** Worker node %s was added with PXE boot (BMC address: %s) ***", arm64Node, bmcAddress)
			}
		} else {
			e2e.Logf("Skipping worker node %s - boot method is %s (BMC: %s), expected %s", arm64Node, bootMethod, bmcAddress, expectedBootMethod)
		}
	}
...
var _ = g.Describe("[OTP][sig-baremetal] IPI BareMetal", func() {
	...
	g.BeforeEach(func() {
		...
		if !(strings.EqualFold(iaasPlatform, "baremetal")) {
			e2e.Logf("Cluster is: %s", iaasPlatform)
			g.Skip("This test is for IPI baremetal cluster!")
		}
	})
