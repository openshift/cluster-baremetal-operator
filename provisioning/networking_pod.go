		...
		ReadinessProbe: &corev1.Probe{
			ProbeHandler: corev1.ProbeHandler{
				Exec: &corev1.ExecAction{
					Command: []string{
						"curl", "-ks", "-o", "/dev/null",
						fmt.Sprintf("https://localhost:%d/", ironicNetworkingRPCPort),
					},
				},
			},
...
func EnsureIronicNetworkingDeployment(ctx context.Context, info *ProvisioningInfo) (updated bool, err error) {
	if !info.ProvConfig.Spec.IsSwitchManagementEnabled() {
		return false, DeleteIronicNetworkingDeployment(ctx, info)
	}
	...
	configAnn, err := secretVersionAnnotation(ctx, info.Client, info.Namespace, "switch-configs", switchConfigsSecretName)
	if err != nil {
		return false, fmt.Errorf("unable to compute switch-configs secret version: %w", err)
	}
	maps.Copy(podAnnotations, configAnn)

	credsAnn, err := secretVersionAnnotation(ctx, info.Client, info.Namespace, "switch-credentials", switchCredentialsSecretName)
	if err != nil {
	...
	deployment, updated, err := resourceapply.ApplyDeployment(ctx,
...
func DeleteIronicNetworkingDeployment(ctx context.Context, info *ProvisioningInfo) error {
	return client.IgnoreNotFound(info.Client.AppsV1().Deployments(info.Namespace).Delete(ctx, ironicNetworkingDeploymentName, metav1.DeleteOptions{}))
}
...
func EnsureIronicNetworkingService(ctx context.Context, info *ProvisioningInfo) (updated bool, err error) {
	if !info.ProvConfig.Spec.IsSwitchManagementEnabled() {
		return false, DeleteIronicNetworkingService(ctx, info)
	}
	...
	_, updated, err = resourceapply.ApplyService(ctx,
...
func DeleteIronicNetworkingService(ctx context.Context, info *ProvisioningInfo) error {
	return client.IgnoreNotFound(info.Client.CoreV1().Services(info.Namespace).Delete(ctx, ironicNetworkingServiceName, metav1.DeleteOptions{}))
}
