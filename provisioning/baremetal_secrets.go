...
func EnsureAllSecrets(ctx context.Context, info *ProvisioningInfo) (bool, error) {
	...
	if err := ensureSwitchConfigSecret(ctx, info); err != nil {
	...
	if err := ensureSwitchCredentialsSecret(ctx, info); err != nil {
...
func ensureNetworkingSecret(ctx context.Context, info *ProvisioningInfo, secretName, dataFileName string) error {
	if !info.ProvConfig.Spec.IsSwitchManagementEnabled() {
...
func ensureSwitchConfigSecret(ctx context.Context, info *ProvisioningInfo) error {
	return ensureNetworkingSecret(ctx, info, switchConfigsSecretName, switchConfigsFileName)
}
...
func ensureSwitchCredentialsSecret(ctx context.Context, info *ProvisioningInfo) error {
	return ensureNetworkingSecret(ctx, info, switchCredentialsSecretName, "")
}
